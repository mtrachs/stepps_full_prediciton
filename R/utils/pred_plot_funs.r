...
plot_pred_maps <- function(r_mean, centers, taxa, t, N, K, T, thresh, limits, type, suff, save_plots, fpath=subDir,height,width){
  ...
  for (k in 1:K){
    prop_dat = rbind(prop_dat, data.frame(props = cut(r_mean[,k], thresh, include.lowest=TRUE, labels=FALSE), 
  ...
  p <- ggplot() + geom_tile(data=prop_dat, aes(x=x, y=y, fill=factor(props))) + 
    #scale_fill_gradientn(colours=tim.colors(length(thresh)), name=bar_title) + coord_fixed() +
    #scale_x_continuous(limits$xlims) + scale_y_continuous(limits$ylims)
    scale_fill_manual(values = tim.colors(length(thresh)), labels=breaklabels, name='Proportion') + 
    scale_colour_manual(values = tim.colors(length(thresh)), labels=breaklabels, name='Proportion') + 
    coord_fixed()  
 # p <- add_map_albers(p, us.shp, limits)#,rescale)
  ...
  Sys.sleep(2)
  if (save_plots){
    ggsave(file=paste(fpath, '/veg_maps_', suff, '.pdf', sep=''), scale=1, width=width, height = height)
#     dev.off()
  }
  return(p)
}
